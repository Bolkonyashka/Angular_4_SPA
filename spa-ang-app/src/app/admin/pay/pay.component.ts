...
export class PayComponent implements OnInit {
  ...
  submit(sf: string, so: string, ff: string, fv: string) {
    ...
    req.open ('GET', '../../../assets/getPay.php?sortfield=' + sf + '&sortorder=' + so + '&filterfield=' + ff + '&filterval=' + fv, true); 
    req.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");
    req.setRequestHeader("Authorization","Bearer " + user.token);//"eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NSIsIm5hbWUiOiJKb2huIEdvbGQiLCJhZG1pbiI6dHJ1ZX0.LIHjWCBORSWMEibq-tnT8ue_deUqZx1K0XxCOXZRrBI"); //this.autser.token);
    req.send();
  }
  ...
  chSafe(csid: string, csst: string, sf: string, so: string, ff: string, fv: string) {
    ...
    req.open ('POST', '../../../assets/chSafe.php', true); 
    req.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");
